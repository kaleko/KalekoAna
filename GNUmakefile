...
#
SUBDIRS := LArSoftImport CCInclusive scratch_ana #ERToolStuff TrackStudy scratch_ana EventFilters #LArSoftImport NuEnergyStuff ERToolStuff SoftwareTrigger RecoEmulator  #ADD_NEW_SUBDIR ... do not remove this comment from this line
#SignalSelection
#UBSensInterface
#CutStudy
#TrackStudy
...
all:
	@for i in $(SUBDIRS); do ( echo "" && echo "Compiling $$i..." && cd $(LARLITE_USERDEVDIR)/KalekoAna/$$i && $(MAKE) ) || exit $$?; done
#####################################################################################
#
# CLEANs...
#
clean:
	@for i in $(SUBDIRS); do ( echo "" && echo "Cleaning $$i..." && cd $(LARLITE_USERDEVDIR)/KalekoAna/$$i && $(MAKE) clean && rm -f $(LARLITE_LIBDIR)/$$i.* ) || exit $$?; done
...
doxygen:
	...
	@mkdir -p $(LARLITE_USERDEVDIR)/KalekoAna/doc/dOxygenMyProject
	@doxygen $(LARLITE_USERDEVDIR)/KalekoAna/doc/doxygenMyProject.script
